...
// ---- AREAS ----
export const listAreasHandler = asyncHandler(async (req, res) => {
    const includeInactive = req.query.includeInactive === "true";
    const where = includeInactive ? {} : { isActive: true };
    const areas = await prisma.area.findMany({ where, orderBy: { name: "asc" } });
    res.json({ success: true, data: areas });
});
export const createAreaHandler = asyncHandler(async (req, res) => {
    if (!req.user)
    ...
    if (existing)
        return res.status(409).json({ success: false, error: "Ya existe un área con ese código" });
    const area = await prisma.area.create({ data: { ...parsed.data, code: parsed.data.code.toUpperCase() } });
    res.status(201).json({ success: true, data: area });
});
export const updateAreaHandler = asyncHandler(async (req, res) => {
    if (!req.user)
        return res.status(401).json({ success: false, error: "No autenticado" });
    const allowed = ["ADMIN", "QUALITY_MANAGER"];
    if (!allowed.includes(req.user.role))
...
export const deleteAreaHandler = asyncHandler(async (req, res) => {
    if (!req.user)
        return res.status(401).json({ success: false, error: "No autenticado" });
    if (req.user.role !== "ADMIN")
        return res.status(403).json({ success: false, error: "Solo Admin puede eliminar" });
    await prisma.area.update({ where: { id: req.params.id }, data: { isActive: false } });
    res.json({ success: true, message: "Área desactivada" });
});
// ---- DOCUMENT TYPES ----
export const listDocumentTypesHandler = asyncHandler(async (req, res) => {
    const includeInactive = req.query.includeInactive === "true";
    const where = includeInactive ? {} : { isActive: true };
    const types = await prisma.documentTypeConfig.findMany({ where, orderBy: { name: "asc" } });
    res.json({ success: true, data: types });
});
export const createDocumentTypeHandler = asyncHandler(async (req, res) => {
    if (!req.user)
        return res.status(401).json({ success: false, error: "No autenticado" });
    const allowed = ["ADMIN", "QUALITY_MANAGER"];
    if (!allowed.includes(req.user.role))
    ...
    if (existing)
        return res.status(409).json({ success: false, error: "Ya existe un tipo con ese código" });
...
export const updateDocumentTypeHandler = asyncHandler(async (req, res) => {
    if (!req.user)
        return res.status(401).json({ success: false, error: "No autenticado" });
    const allowed = ["ADMIN", "QUALITY_MANAGER"];
    if (!allowed.includes(req.user.role))
...
export const deleteDocumentTypeHandler = asyncHandler(async (req, res) => {
    if (!req.user)
        return res.status(401).json({ success: false, error: "No autenticado" });
    if (req.user.role !== "ADMIN")
        return res.status(403).json({ success: false, error: "Solo Admin puede eliminar" });
