...
import { createDocument, uploadDocumentFile, getDocument, updateDocument, submitForReview, approveReview, publishDocument, listDocuments, downloadDocument, createNewVersion, } from "../services/document.js";
...
async function hasAreaAccess(user, documentId, documentArea) {
...
export const uploadFileHandler = asyncHandler(async (req, res) => {
    ...
    const document = await uploadDocumentFile(documentId, req.file.originalname, req.file.buffer, req.user.userId);
    res.json({
        success: true,
        data: document,
    });
...
export const submitForReviewHandler = asyncHandler(async (req, res) => {
    ...
    const document = await submitForReview(documentId, reviewers, comments, req.user.userId);
    res.json({
        success: true,
        data: document,
    });
});
export const approveReviewHandler = asyncHandler(async (req, res) => {
    if (!req.user) {
        return res.status(401).json({ success: false, error: "Not authenticated" });
    }
    ...
    const result = await approveReview(documentId, reviewTaskId, action, comments, req.user.userId);
    res.json({
        success: true,
        data: result,
    });
});
export const publishDocumentHandler = asyncHandler(async (req, res) => {
    if (!req.user) {
        return res.status(401).json({ success: false, error: "Not authenticated" });
    }
    const { documentId } = req.params;
...
export const createNewVersionHandler = asyncHandler(async (req, res) => {
    if (!req.user) {
        return res.status(401).json({ success: false, error: "Not authenticated" });
    }
    const { documentId } = req.params;
