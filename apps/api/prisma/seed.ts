...
const DOCUMENT_TYPES = [
  { code: "PO", name: "Política",             prefix: "PO", requiresElaborado: true,  requiresRevisado: true,  requiresAprobado: true,  description: "Declaración formal de la Alta Dirección que establece principios, compromisos y dirección estratégica del sistema de gestión. No describe procesos ni tareas." },
  { code: "MN", name: "Manual",               prefix: "MN", requiresElaborado: true,  requiresRevisado: true,  requiresAprobado: true,  description: "Documento que describe la estructura general del sistema de gestión, su alcance, procesos e interacciones. Explica cómo está organizado el sistema." },
  { code: "DM", name: "Documento Marco",      prefix: "DM", requiresElaborado: true,  requiresRevisado: true,  requiresAprobado: true,  description: "Documento que establece lineamientos o criterios generales que aplican transversalmente. Traduce la política en directrices." },
  { code: "PR", name: "Procedimiento",        prefix: "PR", requiresElaborado: true,  requiresRevisado: true,  requiresAprobado: true,  description: "Describe cómo se ejecuta un proceso, incluyendo responsabilidades, secuencia y controles." },
  { code: "IT", name: "Instructivo",          prefix: "IT", requiresElaborado: true,  requiresRevisado: true,  requiresAprobado: true,  description: "Documento operativo detallado que explica paso a paso cómo realizar una tarea específica." },
  { code: "RG", name: "Registro",             prefix: "RG", requiresElaborado: true,  requiresRevisado: false, requiresAprobado: true,  description: "Documento que evidencia que una actividad fue realizada. Se completa y archiva." },
  { code: "LI", name: "Listado",              prefix: "LI", requiresElaborado: true,  requiresRevisado: false, requiresAprobado: true,  description: "Documento que enumera elementos bajo control o seguimiento." },
  { code: "PL", name: "Plan",                 prefix: "PL", requiresElaborado: true,  requiresRevisado: false, requiresAprobado: true,  description: "Documento que establece objetivos, responsables y plazos para una actividad puntual y delimitada en el tiempo." },
  { code: "PT", name: "Protocolo",            prefix: "PT", requiresElaborado: true,  requiresRevisado: false, requiresAprobado: true,  description: "Documento técnico que define metodología específica para realizar ensayos, validaciones o pruebas." },
  { code: "IN", name: "Informe",              prefix: "IN", requiresElaborado: true,  requiresRevisado: false, requiresAprobado: true,  description: "Documento que comunica resultados, análisis o conclusiones de una actividad realizada." },
  { code: "FT", name: "Ficha Técnica",        prefix: "FT", requiresElaborado: true,  requiresRevisado: false, requiresAprobado: true,  description: "Documento que describe características técnicas de un producto, insumo o equipo." },
  { code: "DF", name: "Diagrama de Flujo",    prefix: "DF", requiresElaborado: true,  requiresRevisado: true,  requiresAprobado: true,  description: "Representación gráfica de la secuencia de un proceso." },
  { code: "AR", name: "Análisis de Riesgos",  prefix: "AR", requiresElaborado: true,  requiresRevisado: true,  requiresAprobado: true,  description: "Documento que identifica peligros, evalúa riesgos y define medidas de control." },
  { code: "ES", name: "Especificación",       prefix: "ES", requiresElaborado: true,  requiresRevisado: true,  requiresAprobado: true,  description: "Documento que define requisitos técnicos que debe cumplir un producto, proceso o servicio." },
  { code: "CE", name: "Comunicación Externa", prefix: "CE", requiresElaborado: true,  requiresRevisado: false, requiresAprobado: false, description: "Documento formal dirigido a partes externas (proveedores, clientes, autoridades)." },
  { code: "CI", name: "Comunicación Interna", prefix: "CI", requiresElaborado: true,  requiresRevisado: false, requiresAprobado: false, description: "Documento dirigido al personal interno para informar o instruir." },
  { code: "AN", name: "Anexo",                prefix: "AN", requiresElaborado: false, requiresRevisado: false, requiresAprobado: false, description: "Documento complementario que amplía información de otro documento principal. No tiene autonomía funcional." },
  { code: "OG", name: "Organigrama",          prefix: "OG", requiresElaborado: true,  requiresRevisado: false, requiresAprobado: true,  description: "Documento que representa gráficamente la estructura organizativa de la empresa y las relaciones jerárquicas entre sus áreas o sectores." },
  { code: "CT", name: "Contrato",             prefix: "CT", requiresElaborado: true,  requiresRevisado: false, requiresAprobado: false, description: "Documento formal que establece los términos, condiciones y responsabilidades acordadas entre la organización y una o más partes externas para la prestación de servicios, provisión de bienes u otras relaciones comerciales." },
];
...
async function main() {
  ...
  for (const dt of DOCUMENT_TYPES) {
    await prisma.documentTypeConfig.upsert({
      where: { code: dt.code },
      update: { name: dt.name, prefix: dt.prefix, description: dt.description, requiresElaborado: dt.requiresElaborado, requiresRevisado: dt.requiresRevisado, requiresAprobado: dt.requiresAprobado, isActive: true },
