...
echo "Running DB migration (sync - ensures schema is ready before server starts)..."
cd /app/apps/api && npx prisma@5 db push --accept-data-loss || echo "DB push failed, continuing..."

echo "Running seed in background..."
(cd /app && node /app/node_modules/.bin/tsx /app/apps/api/prisma/seed.ts && echo "Seed OK") &
